...
def generate_hospital_report(output_dir: str, metadata: dict, patient_results: dict):
    ...
    print(f"  [HIGH] Cao risco: {report['summary']['high_risk']} bệnh nhân")
    print(f"  [MODERATE] Trung bình: {report['summary']['moderate_risk']} bệnh nhân")
    print(f"  [LOW] Thấp: {report['summary']['low_risk']} bệnh nhân")
    print(f"  [VERY_LOW] Rất thấp: {report['summary']['very_low_risk']} bệnh nhân")

    # Alert for high-risk patients
    if report['summary']['high_risk'] > 0:
        print(f"\n[WARNING] CẢNH BÁO: Có {report['summary']['high_risk']} bệnh nhân nguy cơ cao!")
...
def main():
    ...
    print("HE THONG PHAT HIEN COVID-19 BENH VIEN - TUAN SCAN")
    print("=" * 70)
    print("Tu dong xu ly toan bo du lieu CT scan tuan cua benh vien")
    print("=" * 70)

    # Configuration
    config = {
        "input_weekly_dir": "data/weekly_input",      # Folder chứa dữ liệu tuần
        "working_dir": "data/working",                # Thư mục làm việc
    ...
    if successful_count == len(patients):
        print("[SUCCESS] Tất cả bệnh nhân xử lý thành công!")
        print(f"\n[INFO] Kết quả đã lưu trong: {config['output_dir']}")
        print("   - Mỗi bệnh nhân có folder riêng với visualization")
        print("   - Báo cáo tổng: hospital_report.json")
        return 0
    else:
        print(f"[WARNING] {len(patients) - successful_count} bệnh nhân thất bại")
        return 1
