"""
Clean Parallel COVID-19 Detection Pipeline
4 patients processed with individual component visualization - no Unicode characters
"""
...
def create_visualization_op(
    ...
    stages = [
        ...
        ("Lung Seg", "Segment", "blue", "Completed"),
        ("COVID Detect", "Detect", "orange", "Completed"),
        ("Visualization", "Visual", "purple", "Completed")
    ]
...
@dsl.pipeline(
    name="clean-parallel-covid-pipeline",
    description="Parallel COVID-19 Detection Pipeline - 4 Patients with Individual Component Separation (Clean Version)"
)
def clean_parallel_covid_pipeline(
...
def compile_clean_pipeline():
    """Compile the clean parallel pipeline"""
    print("[COMPILING] Clean parallel COVID-19 detection pipeline...")
    print("This pipeline uses simple string returns to avoid KFP type annotation issues!")

    try:
        # Compile pipeline
        kfp.compiler.Compiler().compile(
            clean_parallel_covid_pipeline,
            "clean_parallel_pipeline.yaml"
        )

        print("[SUCCESS] Clean parallel pipeline compiled successfully!")
        print("[OUTPUT] clean_parallel_pipeline.yaml")

        # Validate the YAML
        import yaml
        with open("clean_parallel_pipeline.yaml", 'r') as f:
        ...
        print("\n" + "="*80)
        print("CLEAN PARALLEL PIPELINE - INDIVIDUAL COMPONENTS")
        print("="*80)
        ...
        print("\n[UI] In Kubeflow UI You Will See:")
        ...
        print("\n" + "="*80)
        print("CLEAN PARALLEL PIPELINE READY!")
        print("="*80)

        return "clean_parallel_pipeline.yaml"
...
if __name__ == "__main__":
    ...
    if result:
        ...
        print("  - Resource optimization")
        print("  - Fixed KFP type annotation issues")
        print("\n[DEPLOYMENT]: Upload to Kubeflow and see the individual component workflow!")
        print("\n[RESOLVED]: KFP output type annotation issue fixed!")
